...
export const serverConfig = {
  ...
  appUrl: warnEnv('NEXT_PUBLIC_APP_URL', process.env.NEXT_PUBLIC_APP_URL) || 'http://localhost:9002',
  cronSecret: optionalEnv('CRON_SECRET'),
  rpcUrl: optionalEnv('RPC_URL'),
  hasDatabaseConfig: !!process.env.DATABASE_URL && !isPlaceholder(process.env.DATABASE_URL || ''),
  hasGeminiConfig: !!process.env.GEMINI_API_KEY && !isPlaceholder(process.env.GEMINI_API_KEY || ''),
  hasTreasuryConfig: !!process.env.TREASURY_PRIVATE_KEY && !isPlaceholder(process.env.TREASURY_PRIVATE_KEY || ''),
  hasQstashConfig: !!process.env.QSTASH_TOKEN && !isPlaceholder(process.env.QSTASH_TOKEN || '') && !!process.env.QSTASH_CURRENT_SIGNING_KEY && !isPlaceholder(process.env.QSTASH_CURRENT_SIGNING_KEY || ''),
  hasCronSecret: !!process.env.CRON_SECRET && !isPlaceholder(process.env.CRON_SECRET || ''),
  hasAppUrl: !!process.env.NEXT_PUBLIC_APP_URL && !isPlaceholder(process.env.NEXT_PUBLIC_APP_URL || ''),
